...
class SetupOptions(object):
    ...
    def _get_notcritical_tags(self):
        """
        Adds options for test with tags marked for selecting as not critical tests.
        """
        self.command += self.add_option_list("noncritical", self.args.notCritical)
    ...
    def get_rerun_xml(self, xml_run):
        ...
        self.command += self.add_option(
            "rerunfailed", self._add_quotes_to_path(path))
    ...
    def tidy_command(self):
        ...
        self.command = python_command + " -m robot.tidy -r \"" +\
            self.args.test_folder + "\""
